...
def getRandomDateTimeBeforeEnrollment(baselineEvent):
    date = datetime.fromisoformat(baselineEvent["incidentDate"]) - timedelta(
...
# def prescribeDrug(typeOfPatient, monthOfTreatment):
#     match typeOfPatient:
#         case "New":
#             if monthOfTreatment in ["1 - 2 Weeks", "2 - 4 Weeks"]:
#                 return {"id" : "Ghsh3wqVTif", "vZMCHh6nEBZ" : "2"}

#             elif monthOfTreatment in ["3 Month", "4 Month", "5 Month", "6 Month"]:
#                 return ["RH", "4"]
...
#         case "Relapse":
#         case "Treatment after Lost to Follow up":
#         case "Treatment after failure":
#         case "Treatment history Unknown":
#################################################################################################
...
class TreatmentDataSynthesizeDataValuesr:
    def __init__(self):
    ...
    def SynthesizeDataValues(self, baselineEvent, monthOfTreatment):
        ...
        #reference date only a
        baselineEventDate = datetime.fromisoformat(baselineEvent["eventDate"])

        period = (datetime.today() - baselineEventDate)
            ...
            match treatmentElement['id']:
                ...
                case "FklL99yLd3h":
                    value = random.choices(
                        ["Facility DOT", "Digital Community DOT", "Non Digital Community DOT"],
...
#############################################################################################
                ...
                case "YwZN88UJ98d":
                    if getValueByID(stageDataValues, "Jl3oWFGGt1U") in ["Severe Acute Malnutrition", "Moderate Acute Malnutrition"]:
                        value = random.choice(["Nutrition Counselling", "Referral", "Therapeutic foods", "Supplementary Foods"])
...
def main():
    synthesizer = TreatmentDataSynthesizeDataValuesr()
